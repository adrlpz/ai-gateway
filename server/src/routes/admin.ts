...
function normalizeTokenLimit(value: unknown): number | null | undefined {
  if (value === undefined) return undefined;
  if (value === null || value === '') return null;
  if (typeof value !== 'number' || !Number.isFinite(value) || value < 0 || !Number.isInteger(value)) {
    throw new Error('token_limit must be a non-negative integer or null');
  }
  return value;
}
...
function usageStatusClause(status: string | undefined): string {
  if (status === 'error') return ' AND status >= 400';
  if (status === 'ok') return ' AND status < 400';
  return '';
}
...
adminRouter.post('/keys', (req, res) => {
  ...
  let tokenLimit: number | null | undefined;
  try {
    tokenLimit = normalizeTokenLimit((req.body as { token_limit?: unknown }).token_limit);
  } catch (err) {
  ...
  const id = uuid();
  db.prepare('INSERT INTO gateway_keys (id, key_hash, key_preview, name, enabled, token_limit, created_at) VALUES (?, ?, ?, ?, 1, ?, ?)')
    .run(id, hash, preview, name ?? null, tokenLimit ?? null, Date.now());
  // Return the raw key ONCE - it won't be shown again
  res.status(201).json({ id, key: rawKey, preview, name, token_limit: tokenLimit ?? null });
});
...
adminRouter.patch('/keys/:id', (req, res) => {
  ...
  if (tokenLimit !== undefined) db.prepare('UPDATE gateway_keys SET token_limit = ? WHERE id = ?').run(tokenLimit, req.params.id);
  res.json({ ok: true });
});

// ─── Usage ────────────────────────────────────────────────────────────────────

adminRouter.get('/usage/keys', (_req, res) => {
  const now = Date.now();
  ...
  const keys = db.prepare(`
    SELECT
      ...
      gk.created_at,
      gk.last_used_at,
      COALESCE(day.requests_24h, 0) as requests_24h,
      COALESCE(day.tokens_24h, 0) as tokens_24h
    ...
    ) day ON day.gateway_key_id = gk.id
    ORDER BY gk.tokens_count DESC, gk.created_at DESC
  `).all(dayAgo) as (GatewayKey & { requests_24h: number; tokens_24h: number })[];

  const summary = {
    active_keys: keys.filter(key => key.enabled).length,
    total_lifetime_tokens: keys.reduce((sum, key) => sum + key.tokens_count, 0),
    total_tokens_24h: keys.reduce((sum, key) => sum + key.tokens_24h, 0),
    limit_reached_keys: keys.filter(key => key.token_limit !== null && key.tokens_count >= key.token_limit).length,
  };
  ...
  res.json({
    ...
    keys: keys.map(key => ({
      ...key,
      limit_reached: key.token_limit !== null && key.tokens_count >= key.token_limit,
    })),
  });
});

adminRouter.get('/usage/keys/:id/logs', (req, res) => {
  const key = db.prepare('SELECT id FROM gateway_keys WHERE id = ?').get(req.params.id) as { id: string } | undefined;
  if (!key) {
  ...
  const statusClause = usageStatusClause(status);

  const total = (db.prepare(`
    SELECT COUNT(*) as c
    FROM request_logs
    WHERE gateway_key_id = ?${statusClause}
  ...
  const logs = db.prepare(`
    ...
    LIMIT ? OFFSET ?
  `).all(req.params.id, limit, offset) as RequestLog[];
