...
export function requireGatewayKey(req: Request, res: Response, next: NextFunction): void {
  ...
  if (record.token_limit !== null && record.tokens_count >= record.token_limit) {
    res.status(429).json({
      error: {
        message: `API key quota exceeded. This key has reached its lifetime limit of ${record.token_limit.toLocaleString()} tokens.`,
        type: 'quota_exceeded',
      },
    });
    return;
