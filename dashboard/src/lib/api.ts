const BASE = '/api';
export const AUTH_ERROR_EVENT = 'ai-gateway-auth-error';
...
export type RequestLog = {
  id: string; gateway_key_id?: string | null; provider_id: string | null; model: string | null;
...
export type KeyUsageOverview = {
  summary: {
    active_keys: number;
    total_lifetime_tokens: number;
    total_tokens_24h: number;
    limit_reached_keys: number;
  };
  keys: KeyUsageSummary[];
};

export type KeyUsageLog = RequestLog;

export type KeyUsageLogsResponse = {
  logs: KeyUsageLog[];
  total: number;
  page: number;
  limit: number;
...
export const api = {
  ...
  keys: {
    ...
    create: (data?: { name?: string; token_limit?: number | null }) =>
      post<{ id: string; key: string; preview: string; token_limit: number | null }>('/keys', data ?? {}),
    delete: (id: string) => del<{ ok: boolean }>(`/keys/${id}`),
    update: (id: string, data: { enabled?: boolean; name?: string; token_limit?: number | null }) =>
      patch<{ ok: boolean }>(`/keys/${id}`, data),
  ...
  usage: {
    keys: () => get<KeyUsageOverview>('/usage/keys'),
    keyLogs: (id: string, params?: { page?: number; limit?: number; status?: string }) => {
      const qs = new URLSearchParams();
      if (params?.page) qs.set('page', String(params.page));
      if (params?.limit) qs.set('limit', String(params.limit));
      if (params?.status) qs.set('status', params.status);
      return get<KeyUsageLogsResponse>(`/usage/keys/${id}/logs?${qs}`);
    },
  },
  models: {
